...
from homeassistant.const import TEMP_CELSIUS, DATA_GIGABYTES
...
async def test_sensor_registry(hass, mqtt_client_mock, mqtt_mock, mock_phoniebox, config):
    ...
    entry: RegistryEntry = entity_registry.async_get("sensor.phoniebox_test_box_version_2")
    assert entry
...
async def test_sensor_registry_ignore_value(hass, mqtt_client_mock, mqtt_mock, mock_phoniebox, config):
    ...
    entry: RegistryEntry = entity_registry.async_get("sensor.phoniebox_test_box_volume_2")
...
async def test_sensor_registry_update(hass, mqtt_client_mock, mqtt_mock, mock_phoniebox, config):
...
async def test_temperature_sensor(hass, mqtt_client_mock, mqtt_mock, mock_phoniebox, config):
...
async def test_player_state_sensor(hass, mqtt_client_mock, mqtt_mock, mock_phoniebox, config):
...
async def test_spotify_source_sensor(hass, mqtt_client_mock, mqtt_mock, mock_phoniebox, config):
    async_fire_mqtt_message(hass, "test_phoniebox/attribute/file", "spotify:playlist-id")
    await hass.async_block_till_done()
...
async def test_file_source_sensor(hass, mqtt_client_mock, mqtt_mock, mock_phoniebox, config):
    async_fire_mqtt_message(hass, "test_phoniebox/attribute/file", "file://path-to-file")
    await hass.async_block_till_done()
